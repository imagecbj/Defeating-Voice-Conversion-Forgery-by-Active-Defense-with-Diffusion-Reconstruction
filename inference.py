...
def predict(model, spectrogram, target_std, global_cond=None, fast_sampling=True):
    with torch.no_grad():
        ...
        audio = torch.randn(spectrogram.shape[0], model.params.hop_samples * spectrogram.shape[-1], device=device) * target_std
        noise_scale = torch.from_numpy(alpha_cum ** 0.5).float().unsqueeze(1).to(device)

        for n in range(len(alpha) - 1, -1, -1):
            c1 = 1 / alpha[n] ** 0.5
            c2 = beta[n] / (1 - alpha_cum[n]) ** 0.5
            audio = c1 * (audio - c2 * model(audio, spectrogram, torch.tensor([T[n]], device=audio.device), global_cond).squeeze(1))
            if n > 0:
...
def main(args):
    ...
    if args.fast:
        if T_OVERRIDE is not None:
            if T_OVERRIDE == 6:
                ...
                NOISE_OVERRIDE = np.linspace(1e-4, 0.05, T_OVERRIDE).tolist()
                print("WARNING: --fast_iter other than [6, 12] is given. Using linear beta schedule: performance is expected to be WORSE!")
            params.inference_noise_schedule = NOISE_OVERRIDE
            print("INFO: inference noise schedule updated, fast_iter {} value {}".format(len(params.inference_noise_schedule), params.inference_noise_schedule))
        else:
    ...
    if args.fast:
        sample_path = os.path.join(dir_parent, 'sample_fast', dir_base + '_step{}_fast_iter{}'.format(step, T_OVERRIDE))
    else:
    ...
    for i, features in tqdm(enumerate(dataset_test)):
        ...
        sample_name = "ae_" + os.path.basename(input_filename)
...
if __name__ == '__main__':
    ...
    parser.add_argument('data_root', help='root of the dataset. used to save the statistics for PriorGrad. example: for LJSpeech, specify /path/to/your/LJSpeech-1.1')
    parser.add_argument('filelist', help='text file containing data path. example: for LJSpeech, refer to ./filelists/test.txt')
    parser.add_argument('--step', type=int, default=3020000, help='number of training step checkpoint to load. If not provided, tries to load the symlinked weights.pt')
    parser.add_argument('--fast', '-f', action='store_true', default=False, help='fast sampling procedure')
    parser.add_argument('--fast_iter', '-t', type=int, default=None, help='number of fast inference diffusion steps for sampling. 6, 12, and 50 steps are officially supported. If other value is provided, linear beta schedule is used.')
