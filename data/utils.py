...
def get_melgan_mel_tensor(audio, is_transposed=False, is_squeeze=True):
    """
    
    
    
    - audio: torch.Tensor [batch_size, length]  [length]
    - is_transposed:  False  [batch_size, n_mels, T]
    - is_squeeze: 
    
    
    - melgan_mel:  [batch_size, n_mels, T]
    """
    if len(audio.shape) == 1:
        ...
        raise ValueError("Audio tensor must be 1D, 2D, or 3D.")
    
    audio = audio.to(hp.device)  # 
    
    melgan_mel = audio2mel(audio)  #  audio2mel 
