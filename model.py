...
class PriorGrad(nn.Module):
    ...
    def forward(self, audio, spectrogram, diffusion_step, global_cond=None):
        #  audio 
        if audio.dim() == 2:
            x = audio.unsqueeze(1)  #  (batch_size, sequence_length)  (batch_size, 1, sequence_length)
        elif audio.dim() == 3:
            x = audio  #  3D  (batch_size, channels, sequence_length)
        else:
            raise ValueError(f"Expected audio tensor to be 2D or 3D, but got {audio.dim()}D tensor.")
