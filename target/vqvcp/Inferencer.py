...
class VQVCPInferencer(object):
    ...
    def inference_from_mel(self, tgt_mel, src_mel):
        ...
        return cvt_mel
# Mel tgt_mel Mel src_mel Mel cvt_mel
    def inference_from_path(self, tgt_path, src_path, save_folder):
        ...
        #  get_melgan_mel
        melgan_src_mel = get_melgan_mel(str(src_path), is_squeeze=False, is_transposed=False).to(hp.device)
        melgan_tgt_mel = get_melgan_mel(str(tgt_path), is_squeeze=False, is_transposed=False).to(hp.device)
